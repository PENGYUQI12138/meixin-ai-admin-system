...
2. 搜索并打开「美心行政」工作台，从入口进入真实的档案列表，点击「新建」。先建课程、教师、教室和学生。学生年级、课程学科使用标准选项；学生的就读学校可选，监护人电话为建档必填项。
3. 在「排课」中新建，选择课程、教师、教室和开始时间，并添加至少一名学生。结束时间尚未人工填写时，系统按课程默认时长自动计算；人工改过结束时间后不会继续强制覆盖。「保存」是草稿，不占用资源；「提交」才确认排课。
