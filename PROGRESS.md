...
1. 创建阶段 4 Git checkpoint。
2. 进入阶段 5：补齐 M2 权限、数据库唯一约束和回滚边界复核，运行 M1 30 项 + M2 全量回归及隔离数据清理检查。
3. 恢复真实浏览器通道后完成 Scheduler / Manager 关键点击路径与控制台验收；不得把通道故障记为验收通过。
4. 阶段 6 形成候选稳定 checkpoint；阶段 7 正式部署前先完整备份，并说明 migrate、容器、停机与回滚影响，等待用户确认。
