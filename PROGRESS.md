...
- 浏览器控制通道再次返回 `nodeRepl.fetch request failed`。自动化和源码审查已完成，但 Scheduler / Manager 真实点击与控制台验收仍是阶段 5 唯一未完成门禁，详见 `docs/M2_ACCEPTANCE.md`。
...
1. 为阶段 5 审查修复与自动化证据创建 checkpoint。
2. 恢复真实浏览器通道后完成 Scheduler / Manager 关键点击路径与控制台验收；不得把通道故障记为验收通过。
3. 浏览器门禁通过后，阶段 6 形成候选稳定 checkpoint。
4. 阶段 7 正式部署前先完整备份，并说明 migrate、容器、停机与回滚影响，等待用户确认。
